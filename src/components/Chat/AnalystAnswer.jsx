...
export const proseComponents = {
    ...
    a: ({ href, children }) => (
        <a href={href} target="_blank" rel="noopener noreferrer"
           className="underline underline-offset-2 font-semibold text-zinc-900 dark:text-[#FDD405]">{children}</a>
    ),
    table: ({ children }) => (
        ...
        <div className="mt-3 rounded-xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50 dark:bg-[#0d0c0b] px-3.5 py-3">
            <button onClick={() => setOpen(o => !o)}
                ...
                <span className="flex items-center gap-2 min-w-0">
                    <MiniLabel>{label}</MiniLabel>
                    {badge && (
                        <span className="text-[9px] px-1.5 py-0.5 rounded-full font-semibold bg-zinc-200/70 text-zinc-600 dark:bg-[#FDD405]/10 dark:text-[#FDD405]/90 flex-shrink-0">
                            {badge}
                        </span>
                    )}
                </span>
...
const DocLink = ({ url, children }) => url ? (
    <a href={url} target="_blank" rel="noopener noreferrer"
       className="inline-flex items-start gap-1 text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-[#FDD405] hover:underline transition-colors">
        <span className="flex-1 min-w-0">{children}</span>
                    ...
                    {ari.pdf_url && (
                        <a href={ari.pdf_url} target="_blank" rel="noopener noreferrer"
                           className="mt-2.5 inline-flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-[0.12em]
                                      text-zinc-900 dark:text-[#FDD405] hover:underline">
                            ...
                            return (
                                <div key={g.type || g.label}>
                                    <div className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 mb-1.5 inline-flex items-center gap-1.5">
                                        <Icon size={13} strokeWidth={2} className="text-zinc-700 dark:text-[#FDD405]" />
                                        ...
                                        {(Array.isArray(g.items) ? g.items : []).map((it, i) => {
                                            ...
                                            return it.url ? (
                                                <a key={i} href={it.url} target="_blank" rel="noopener noreferrer"
                                                   title={it.title}
                                                   className="text-[11px] px-2.5 py-1 rounded-lg font-medium border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:border-zinc-400 dark:hover:border-[#FDD405]/60 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
